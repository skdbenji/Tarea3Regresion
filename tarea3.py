...
from datos import (
    ...
    tarjetas_cc, tarjetas_pal, expulsados_cc, expulsados_pal,
    local_cc, anios
)
...
def preparar_datos():
    ...
    X = np.array([
        [tarjetas_cc[i], tarjetas_pal[i], expulsados_cc[i], expulsados_pal[i], local_cc[i], anios[i]]
        for i in range(len(cc_todos))
    ])

    # Aquí podría ir la limpieza de tipeado si es que lo hacen con código, si es a mano borren esto 
...
def calcular_estadisticas_basicas(cc_todos, pal_todos, dif_goles):
    ...
    print(f"Goles Colo-Colo -> Media: {datos['Goles_CC'].mean():.2f} | Mediana: {datos['Goles_CC'].median()} | Moda: {datos['Goles_CC'].mode()[0]}")
    print(f"Goles Palestino -> Media: {datos['Goles_PAL'].mean():.2f} | Mediana: {datos['Goles_PAL'].median()} | Moda: {datos['Goles_PAL'].mode()[0]}")
    print(f"Diferencia Goles -> Media: {datos['Dif_Goles'].mean():.2f} | Mediana: {datos['Dif_Goles'].median()} | Moda: {datos['Dif_Goles'].mode()[0]}")
...
def entrenar_modelos(X, Y_cc, Y_pal, dif_goles, Y_gana):
    # DIVIDIR DATOS
    (
        X_train, X_test, Ycc_train, Ycc_test, Ypal_train, Ypal_test,
        Ydif_train, Ydif_test, Ygana_train, Ygana_test
    ) = train_test_split(X, Y_cc, Y_pal, dif_goles, Y_gana, test_size=0.3, random_state=42)
    ...
    print(f"MSE Colo-Colo : {mean_squared_error(Ycc_test, modelo_cc.predict(X_test)):.3f}")

    print(f"R² Palestino : {modelo_pal.score(X_test, Ypal_test):.3f}")
    print(f"MSE Palestino : {mean_squared_error(Ypal_test, modelo_pal.predict(X_test)):.3f}")

    print(f"R² Diferencia goles : {modelo_dif.score(X_test, Ydif_test):.3f}")
    print(f"MSE Diferencia goles : {mean_squared_error(Ydif_test, modelo_dif.predict(X_test)):.3f}")

    print(f"Precision victorias CC : {modelo_gana.score(X_test, Ygana_test):.3f}")

    return modelo_cc, modelo_pal, modelo_dif, modelo_gana, X_test, Ygana_test
...
def predecir_nuevo_partido(modelo_cc, modelo_pal, modelo_dif, modelo_gana):
...
def generar_graficos(datos, cc_todos, pal_todos, dif_goles, modelo_gana, X_test, Ygana_test):
    ...
    # EVOLUCION HISTORICA
    # DIFERENCIA DE GOLES
    plt.scatter(datos["Año"], dif_goles, alpha=0.6, color='seagreen')
    plt.axhline(0, color='red', linestyle='--')
    # TITULO
    plt.title("Evolucion Historica: Diferencia de Goles a traves de los AÑOs")
    ...
    sns.boxplot(x=datos["Local_CC"], y=dif_goles, palette="Set2")
...
if __name__ == "__main__":
    ...
    datos_completos = calcular_estadisticas_basicas(cc_todos, pal_todos, dif_goles)
    
    # 3. Entrenar y evaluar modelos
    mod_cc, mod_pal, mod_dif, mod_gana, X_test, Ygana_test = entrenar_modelos(X, Y_cc, Y_pal, dif_goles, Y_gana)
    
    # 4. Mostrar probabilidades y predecir
    mostrar_probabilidades(cc_todos, pal_todos, dif_goles)
    predecir_nuevo_partido(mod_cc, mod_pal, mod_dif, mod_gana)
    
    # 5. Dibujar los gráficos
    generar_graficos(datos_completos, cc_todos, pal_todos, dif_goles, mod_gana, X_test, Ygana_test)
